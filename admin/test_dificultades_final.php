...
<body>
    <div class="container mt-4">
        ...
        if (isset($_POST['ejecutar_test'])) {
            // Solo incluir la conexión a la base de datos, NO guardar_estudiante.php
            ...
            try {
                $sql = "INSERT INTO estudiantes (nombres, apellido_paterno, apellido_materno, carnet_identidad, fecha_nacimiento, genero) 
                        VALUES (?, ?, ?, ?, ?, ?)";
            ...
            // Paso 2: Guardar información de dificultades (función local SIN redirecciones)
            echo '<div class="alert alert-info mt-3">
                    <h6><i class="bi bi-2-circle"></i> Guardando información de dificultades...</h6>';
            
            try {
                // Función local que simula exactamente lo que hace guardarInformacionSecundaria para dificultades
                if (!empty($testData['dif_tiene_dificultad']) && $testData['dif_tiene_dificultad'] === 'Si') {
                    ...
                    $stmt->execute([
                        $id_estudiante,
                        $testData['dif_tiene_dificultad'],
                        $testData['dif_auditiva'] ?? 'No',
                        $testData['dif_visual'] ?? 'No',
                        $testData['dif_intelectual'] ?? 'No',
                        $testData['dif_fisico_motora'] ?? 'No',
                        $testData['dif_psiquica_mental'] ?? 'No',
                        $testData['dif_autista'] ?? 'No'
                    ]);
                }
            ...
            try {
                ...
                $stmt->execute([
                    ...
                    $testDataSinDificultades['apellido_paterno'],
                    $testDataSinDificultades['apellido_materno'],
                ...
                // Intentar guardar dificultades (no debería crear registro porque dif_tiene_dificultad = 'No')
                if (!empty($testDataSinDificultades['dif_tiene_dificultad']) && $testDataSinDificultades['dif_tiene_dificultad'] === 'Si') {
                    // Este código no se ejecutará porque el valor es 'No'
                }
