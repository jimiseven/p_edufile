...
<body>
    ...
    <script>
        function generarBoletinesPDF() {
            ...
            const materiasPadre = Object.values(materiasPadreObj);
            ...
            const doc = new jsPDF({
                ...
                unit: 'pt',
                format: [612.0, 504.57] // 21.59cm x 17.8cm en landscape
            });
            ...
            estudiantes.forEach((estudiante, index) => {
                ...
                // Logo
                doc.addImage(LOGO_BASE64, 'PNG', 550, 20, 40, 40);

                // Título
                doc.setFontSize(16);
                doc.setFont('helvetica', 'bolditalic');
                doc.text("U.E SIMON BOLIVAR", 306, 40, {
                    align: 'center'
                });
                ...
                doc.text("ESTUDIANTE :", 40, 70);
                doc.setFont('helvetica', 'normal');
                doc.text(`${estudiante.apellido_paterno} ${estudiante.apellido_materno} ${estudiante.nombres}`, 120, 70);

                doc.setFont('helvetica', 'bold');
                doc.text("CURSO :", 40, 85);
                doc.setFont('helvetica', 'normal');
                doc.text(`${nombreCurso} PRIMARIA`, 120, 85);
                ...
                const ordenDeseado = [
                    "LENGUAJE", "INGLES", "CIENCIAS SOCIALES",
                    "EDUCACION FISICA Y DEPORTES", "EDUCACION MUSICA",
                    "ARTES PLASTICAS", "MATEMATICAS", "TECNICA TECNOLOGIA",
                    "CIENCIAS NATURALES", "VALORES ESPIRITUALIDAD Y RELIGIONES"
                ];
                ...
                const rows = todasMaterias.map(materia => {
                    ...
                    const promedio = validas.length ? (validas.reduce((a, b) => a + b, 0) / validas.length).toFixed(2) : '-';
                ...
                const totalTableWidth = 230 + 55 + 55 + 55 + 65; // suma total = 460
                const pageWidth = 612; // tamaño total en puntos
                ...
                doc.setFont('helvetica', 'bold');
                doc.text("FIRMA MAESTRO", 150, yFinal, {
                    align: 'center'
                });
                doc.text("DIRECCIÓN", 470, yFinal, {
                    align: 'center'
                });
            });
