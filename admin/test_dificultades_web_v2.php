...
<body>
    <div class="container mt-4">
        ...
        if (isset($_POST['ejecutar_test'])) {
            ...
            // Paso 2: Guardar información de dificultades (usando función local)
            echo '<div class="alert alert-info mt-3">
                    <h6><i class="bi bi-2-circle"></i> Guardando información de dificultades...</h6>';
            
            try {
                // Función local para guardar dificultades (sin redirecciones)
                function guardarDificultadesTest($conn, $id_estudiante, $postData) {
                    // Dificultades
                    if (!empty($postData['dif_tiene_dificultad']) && $postData['dif_tiene_dificultad'] === 'Si') {
                        ...
                        $stmt->execute([
                            ...
                            $postData['dif_auditiva'] ?? 'No',
                            $postData['dif_visual'] ?? 'No',
                            $postData['dif_intelectual'] ?? 'No',
                            $postData['dif_fisico_motora'] ?? 'No',
                            $postData['dif_psiquica_mental'] ?? 'No',
                            $postData['dif_autista'] ?? 'No'
                        ]);
                    }
                }
                
                guardarDificultadesTest($conn, $id_estudiante, $testData);
            ...
            try {
                ...
                guardarDificultadesTest($conn, $id_estudiante2, $testDataSinDificultades);
